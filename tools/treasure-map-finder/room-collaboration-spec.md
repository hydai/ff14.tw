...
- 不儲存敏感資料
- 24 小時自動過期機制
- 基於信任的設計，嘗試提供基本權限控制但存在根本限制
- 成員 ID 使用 crypto.randomUUID() 生成，確保唯一性
- CORS 白名單限制，只允許 ff14.tw 相關網域
- 資料歸屬完整性，避免本地寶圖被錯誤歸屬
- **重要安全限制**：系統無身份驗證機制，存在以下安全漏洞
  - **離開房間**：任何知道成員 ID 的人都可移除該成員
  - **更新暱稱**：任何知道成員 ID 的人都可修改該成員暱稱
  - **移除成員**：雖限制為房主功能，但 creatorId 公開可見，任何人都可冒充房主
  - **根本問題**：所有成員 ID 和 creatorId 都在 API 回應中公開，無法有效防止濫用
  - **設計取捨**：系統依賴房間代號的私密性和使用者的善意，接受這些安全風險以簡化使用體驗
